...
foreach( $tags as $tag ){
	...
	foreach( $tag->articleIds as $articleId ){
		$link	= UI_HTML_Tag::create( 'a', $articles[$articleId]->title, array( 'href' => './manage/catalog/bookstore/article/edit/'.$articleId ) );
...
extract( $view->populateTexts( array( 'top', 'bottom', 'index' ), 'html/manage/catalog/bookstore/tag/' ) );
