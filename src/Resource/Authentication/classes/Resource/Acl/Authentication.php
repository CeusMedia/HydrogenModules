...
class Resource_Acl_Authentication extends Abstraction
{
	...
	public function setRight( int|string $roleId, string $controller, string $action ): int
	{
		...
		if( $this->hasNoAccess( $roleId ) )
			return -2;
		return $this->logic->add( $roleId, $controller, $action );
	}
	...
	protected function getRights( $roleId ): array
	{
		...
		if( !isset( $this->rights[$roleId] ) ){
			$this->rights[$roleId]	= [];
			foreach( $this->logic->getRights( $roleId ) as $right ){
	...
	protected function getRole( int|string $roleId ): object|array
	{
		if( !$roleId )
			return [];
		if( !$this->roles )
			foreach( Logic_Role::getInstance( $this->env )->getAll() as $role )
...
/*	protected function listActions( $controller ): array
	{
		return $this->logic->listControllerActions( string $controller );
	}*/

	protected function listControllers(): array
	{
		return $this->logic->listControllers();
	}
