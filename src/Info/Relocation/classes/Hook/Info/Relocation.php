...
class Hook_Info_Relocation extends Hook
{
	...
	{
		if( !$this->env->getModules()->has( 'Server_Router' ) )							//  router module is NOT installed
			return;

		$config	= $this->env->getConfig()->getAll( 'module.info_relocation.', TRUE );	//  shortcut config
		if( !$config->get( 'shortcut' ) )														//  shortcut is NOT enabled
			return;

		$model	= new Model_Route( $this->env );													//  get router module
		if( $model->getByIndex( 'target', 'info/relocation/$1' ) )						//  shortcut route is already set up
			return;
