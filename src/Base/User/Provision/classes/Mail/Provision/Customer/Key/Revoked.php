...
//		$mailData['config']		= $this->env->getConfig()->getAll();
		...
		$this->setSubject( vsprintf( $words['mail-subjects']['keyRevoked'], [
			$data['product']->title,
	...
	{
		return $this->loadContentFile( 'mail/provision/customer/key/revoked.html', $data ) ?? '';
	}
	...
	{
		return $this->loadContentFile( 'mail/provision/customer/key/revoked.txt', $data ) ?? '';
	}
}
