...
$optGender	= UI_HTML_Elements::Options( $optGender, $data->get( 'gender' ) );
...
if( $groups ){
	...
	foreach( $groups as $group ){
		...
		$label  = UI_HTML_Tag::create( 'label', $checkbox.'&nbsp;<span class="bs4-form-check-label">'.$group->title.'</span>', array( 'class' => 'bs2-checkbox bs4-form-check' ) );
		$list[] = UI_HTML_Tag::create( 'li', $label );
	}
	if( count( $list ) >= 2 ){
		$list	= UI_HTML_Tag::create( 'ul', $list, array( 'class' => 'bs2-unstyled bs3-list-unstyled bs4-list-unstyled newsletter-topic-list' ) );
