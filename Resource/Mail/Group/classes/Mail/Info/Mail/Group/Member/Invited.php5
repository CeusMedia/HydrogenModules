<?php
class Mail_Info_Mail_Group_Member_Invited extends Mail_Abstract
{
...
//		$wordsMails	= $this->env->getLanguage()->getWords( 'auth/local', 'mails' );
		...
		$this->setSubject( 'Einladung in der Gruppe "'.$data['group']->title.'"' );
		...
		$plain	= $this->view->loadContentFile( 'mail/info/mail/group/member/invited.txt', $data );
		$this->setText( $plain );
		return $this;
	}
}
